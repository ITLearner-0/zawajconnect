...
-- 1. compatibility_questions - drop existing and recreate with stricter verification
DROP POLICY IF EXISTS "Verified users can access active questions" ON public.compatibility_questions;
...
-- 2. supervision_logs - drop existing and recreate with better verification
DROP POLICY IF EXISTS "Family members can view logs for their supervised users" ON public.supervision_logs;
...
-- 3. conversation_participants - drop existing and recreate with better security
DROP POLICY IF EXISTS "Family members can add themselves as participants" ON public.conversation_participants;
DROP POLICY IF EXISTS "Family members can view supervised conversation participants" ON public.conversation_participants;
DROP POLICY IF EXISTS "Family members can view supervised participants" ON public.conversation_participants;
DROP POLICY IF EXISTS "Users can add participants to their matches" ON public.conversation_participants;
DROP POLICY IF EXISTS "Users can update participants in their matches" ON public.conversation_participants;
DROP POLICY IF EXISTS "Users can view participants in their matches" ON public.conversation_participants;
...
-- 4. Add table for family_notifications if not exists
...
-- 5. family_reviews - drop existing and recreate with higher security
DROP POLICY IF EXISTS "Family members can create their own reviews" ON public.family_reviews;
DROP POLICY IF EXISTS "Family members can update their own reviews" ON public.family_reviews;
DROP POLICY IF EXISTS "Family members can view reviews for their supervised users" ON public.family_reviews;
...
-- 6. video_calls - drop existing and recreate with verification
DROP POLICY IF EXISTS "Users can create video calls for their matches" ON public.video_calls;
DROP POLICY IF EXISTS "Users can update their own video calls" ON public.video_calls;
...
-- 7. family_contact_secure - drop existing and recreate with ultra-strict access
DROP POLICY IF EXISTS "Ultra secure family contact access" ON public.family_contact_secure;
...
-- Phase 4: Create rate limiting function for sensitive operations
...
-- Create rate limiting trigger only for INSERT/UPDATE operations
