...
serve(async (req) => {
  ...
  try {
    ...
    const { data: familyMembers } = await supabase
      .from('family_members')
      .select('id, user_id, invited_user_id, full_name, is_wali, notify_on_calls, max_call_duration_minutes')
    ...
    for (const wali of familyMembers) {
      ...
      if (eventType === 'started') {
        ...
        const durationText = duration ? `${Math.floor(duration / 60)}:${(duration % 60).toString().padStart(2, '0')}` : 'inconnue';
