# Stripe Appeal - Brief Version (Under 5000 Characters)

**Account ID:** acct_1RaFbcKwsHyCjkzy  
**Company:** ZawajConnect (Mariage-Halal)

---

## APPEAL: Muslim Matrimonial Platform - NOT a Dating Service

Dear Stripe Team,

We respectfully request reconsideration of the account closure decision. **ZawajConnect is a Muslim matrimonial platform, not a dating service.** This is a fundamental distinction.

### KEY DIFFERENCES FROM DATING SERVICES

**Dating Services (Prohibited):**
- Casual relationships, no commitment
- No family involvement
- Various relationship types

**Our Matrimonial Platform:**
- **EXCLUSIVE objective: Islamic marriage (Nikah)**
- **MANDATORY family guardian (Wali) system**
- **Family supervision of all interactions**
- **Strict Islamic compliance**

### UNIQUE SAFEGUARDS

**1. Mandatory Wali System**
- Every user MUST invite their legal guardian (parent/family)
- Guardians have full access to profiles and conversations
- No communication possible without family approval
- Dedicated parental supervision dashboard

**2. Strict Islamic Moderation**
- AI-powered content moderation
- Real-time detection of inappropriate content
- Automatic sanctions for violations
- Zero tolerance for non-matrimonial interactions

**3. Identity Verification**
- Verification with official documents
- Photo verification for authenticity
- Visible verification badges

**4. Limited Features**
- No free chat - all conversations supervised
- Limited simultaneous conversations
- Automatic closure after inactivity
- Contact details sharing restricted until family approval

### COMPARABLE TO ACCEPTED PLATFORMS

Similar Muslim matrimonial services using Stripe:
- **Muzz** (formerly Muzmatch)
- **SingleMuslim**
- **Salams**
- **Qiran**

**Our model is identical or stricter than these accepted platforms.**

### REGULATORY COMPLIANCE

- **GDPR**: Full data protection compliance
- **DSA** (Digital Services Act): Content moderation compliance
- **PSD2**: European payment compliance
- **TLS Encryption** and strong authentication
- **SOC 2 certified hosting** (Supabase)

### PUBLIC DOCUMENTATION

All legal documents publicly available:

1. **Terms of Service**: https://mariage-halal.lovable.app/terms
   - Explicit "matrimonial platform" designation
   - Clear prohibition of casual dating

2. **Privacy Policy**: https://mariage-halal.lovable.app/privacy
   - Family data protection
   - Guardian involvement transparency

3. **Refund Policy**: https://mariage-halal.lovable.app/refund
   - 7-day satisfaction guarantee
   - EU withdrawal rights compliance

4. **Community Guidelines**: https://mariage-halal.lovable.app/community-guidelines
   - Strict Islamic behavior rules
   - Violation sanctions

### TRANSPARENT BUSINESS MODEL

**Freemium Subscription Model:**
- Free: Limited basic access
- Premium: €29.99/month - Advanced features
- VIP: €49.99/month - Priority access

**Financial Transparency:**
- Estimated refund rate: <5%
- No disputes expected
- Clear refund policy
- Responsive customer support (<24h)

### FRAUD PREVENTION

- Mandatory identity verification for premium access
- AI-powered fake profile detection
- Reporting system and sanctions
- Rate limiting for suspicious activity
- **Minimum age: 18 years strictly enforced**

### TARGET AUDIENCE

- **French-speaking Muslims** seeking halal marriage
- **Average age**: 25-40 years
- **Intent**: Serious Islamic marriage (Nikah)
- **NOT seeking**: Casual dating, hookups, or uncommitted relationships

### WHY WE ARE COMPLIANT

1. **Not a "dating service"** - We are a family-supervised matrimonial platform
2. **Ethical and transparent model** - Exclusive marriage objective
3. **Comparable to accepted services** - Same model as Muzz, SingleMuslim, Salams
4. **Full regulatory compliance** - GDPR, DSA, PSD2
5. **Complete documentation** - All policies publicly accessible
6. **Robust security** - Identity verification, AI moderation, family oversight

### FORMAL REQUEST

We believe this account closure is based on miscategorization. We are NOT a dating service in the sense of Stripe's Restricted Businesses list. We are a religious, family-oriented matrimonial platform with:

✅ Mandatory family supervision  
✅ Exclusive marriage objective  
✅ Strict Islamic compliance  
✅ AI-powered moderation  
✅ Identity verification  
✅ Comparable to Stripe-accepted platforms  

**We respectfully request a review of this decision** and are available for any clarification, documentation, or platform demonstration.

### CONTACT

- **Website**: https://mariage-halal.lovable.app
- **Email**: contact@mariage-halal.lovable.app
- **Available for**: Call, video conference, or platform demonstration

Thank you for your consideration.

**The ZawajConnect Team**  
Mariage-Halal

---

**Character Count: ~4,800** ✅ (Under 5000 limit)
