...
export function generateReport(messages: Message[]): MonitoringReport {
  ...
  const { score: islamicComplianceScore, violations: islamicViolations } =
    analyzeIslamicCompliance(messages);
  ...
  if (
    ...
    (behavioralScore < 95 || islamicComplianceScore < 95 || sentimentScore < 60)
  ) {
    recommendations.push(
      'Consider maintaining a respectful and positive tone in your conversations'
    );
  }
