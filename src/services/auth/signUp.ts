...
export const signUp = async (data: SignUpData, t: (key: string) => string) => {
  const { email, password, firstName, lastName, gender, waliName, waliRelationship, waliContact } =
    data;
  ...
  try {
    ...
    if (error) {
      ...
      if (
        ...
        toast.error('Limite de fréquence atteinte', {
          description:
            "Trop de tentatives d'inscription récentes. Veuillez attendre quelques minutes avant de réessayer.",
        });
      } else {
    ...
    if (userData?.user) {
      ...
      toast.success('🎉 Compte créé avec succès !', {
        description:
          'Votre compte a été créé. Un email de confirmation a été envoyé à votre adresse.',
        duration: 6000,
      });
      ...
      setTimeout(() => {
        toast.info('📧 Vérification requise', {
          description:
            'Veuillez vérifier votre boîte email et cliquer sur le lien de confirmation pour activer votre compte. Vérifiez aussi vos spams si nécessaire.',
          duration: 10000,
        });
      }, 1500);
    }
