...
export const generateMultipleComparisonPDF = async (options: MultipleComparisonPDFOptions): Promise<void> => {
  ...
  const avgRating = comparisons
    .filter(c => c.rating !== null)
    .reduce((sum, c) => sum + (c.rating || 0), 0) / comparisons.filter(c => c.rating !== null).length;
  ...
  comparisons.forEach((comparison, compIndex) => {
    ...
    comparison.data.profiles.forEach((profile, idx) => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.text(`${profile.age} ans • ${profile.location} • ${profile.occupation}`, margin + 5, currentY);
      currentY += 7;
      
      if (score) {
        doc.setFontSize(9);
        doc.text(`Compatibilité: ${score.overall}% (I:${score.islamic}% C:${score.cultural}% P:${score.personality}%)`, margin + 5, currentY);
      }
      currentY += 12;
    });
    ...
    comparison.data.scores.forEach((score, idx) => {
      ...
      ['overall', 'islamic', 'cultural', 'personality'].forEach((key, barIdx) => {
        ...
        const label = key === 'overall' ? 'Global' : 
                     key === 'islamic' ? 'Islamique' : 
                     key === 'cultural' ? 'Culturel' : 'Personnalité';
        const colors: [number, number, number][] = [[16, 185, 129], [59, 130, 246], [168, 85, 247], [251, 191, 36]];
