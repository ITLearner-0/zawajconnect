...
export class SuspiciousActivityDetector {
  ...
  private static getUserActivities(userId: string): ActivityPattern[] {
    ...
    return this.activityBuffer.filter(
      (activity) => activity.userId === userId && activity.timestamp >= windowStart
    );
  }
  ...
  private static detectUnusualPatterns(activities: ActivityPattern[]): {
    score: number;
    reasons: string[];
  } {
    let score = 0;
  ...
  private static analyzeSuspiciousMetadata(activities: ActivityPattern[]): {
    score: number;
    reasons: string[];
  } {
    let score = 0;
  ...
  private static async handleSuspiciousActivity(
    ...
    try {
      ...
      await (supabase as any).from('security_events').insert({
        ...
        description: `Suspicious activity detected - Threat level: ${threatLevel.level}`,
