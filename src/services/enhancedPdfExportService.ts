...
class EnhancedPdfExportService {
  ...
  private async addSummaryPage(insights: CompatibilityInsights): Promise<void> {
    ...
    this.doc.setFont('helvetica', 'normal');
    const summaryLines = this.doc.splitTextToSize(insights.summary, this.pageWidth - (2 * this.margin));
    ...
    this.doc.setTextColor(60, 60, 60);
    this.doc.setFont('helvetica', 'normal');
    const styleLines = this.doc.splitTextToSize(insights.relationshipStyle, this.pageWidth - (2 * this.margin));
  ...
  private addSuggestionsPage(insights: CompatibilityInsights): void {
    ...
    insights.suggestions.forEach((suggestion) => {
      ...
      this.doc.setFontSize(10);
      this.doc.setFont('helvetica', 'bold');
      const symbol = suggestion.priority === 'high' ? '!' : suggestion.priority === 'medium' ? '•' : 'i';
      ...
      const badgeText = suggestion.priority === 'high' ? 'Haute' : 
      ...
      this.doc.setTextColor(100, 100, 100);
      this.doc.setFont('helvetica', 'normal');
      const descLines = this.doc.splitTextToSize(suggestion.description, this.pageWidth - (2 * this.margin) - 40);
  ...
  private addRedFlagsPage(insights: CompatibilityInsights): void {
    ...
    this.doc.text('Ces points méritent votre attention pour améliorer votre recherche.', this.margin, this.yPosition);
    this.yPosition += 15;
    ...
    insights.redFlags.forEach((flag) => {
      ...
      this.doc.setLineWidth(0.5);
      this.doc.roundedRect(this.margin, boxY, this.pageWidth - (2 * this.margin), boxHeight, 3, 3, 'S');
      ...
      this.doc.setFontSize(8);
      this.doc.setFont('helvetica', 'bold');
      const severityText = flag.severity === 'critical' ? 'Critique' :
                          flag.severity === 'important' ? 'Important' : 'Attention';
      ...
      this.doc.setFont('helvetica', 'normal');
      const descLines = this.doc.splitTextToSize(flag.description, this.pageWidth - (2 * this.margin) - 12);
  ...
  private addIslamicGuidancePage(insights: CompatibilityInsights): void {
    ...
    insights.islamicGuidance.slice(0, 3).forEach((guidance) => {
      ...
      this.doc.setTextColor(60, 60, 60);
      this.doc.setFont('helvetica', 'italic');
      const verseLines = this.doc.splitTextToSize(`"${guidance.verse}"`, this.pageWidth - (2 * this.margin) - 10);
      ...
      this.doc.setTextColor(80, 80, 80);
      this.doc.setFont('helvetica', 'normal');
      const appLines = this.doc.splitTextToSize(guidance.application, this.pageWidth - (2 * this.margin) - 10);
      ...
      this.doc.roundedRect(this.margin, boxStartY - 5, this.pageWidth - (2 * this.margin), boxEndY - boxStartY, 2, 2, 'S');
