...
export class BehaviorAnalyzer {
  ...
  private static readonly INAPPROPRIATE_WORDS = [
    'meet alone', 'secret', 'private meeting', 'don\'t tell',
    'bypass', 'avoid wali', 'without permission'
  ];
  ...
  private static detectRapidMessaging(messages: Message[]): BehaviorPattern | null {
    ...
    const timeSpan = new Date(lastMsg.created_at).getTime() - 
  ...
  private static detectSpam(messages: Message[]): BehaviorPattern | null {
    const spamIndicators = [
      'click here', 'visit my profile', 'check this out',
      'limited time', 'act now', 'free money'
    ];
  ...
  private static calculateScores(patterns: BehaviorPattern[], messageCount: number): Omit<BehaviorScore, 'patterns'> {
    let baseScore = 100;
  ...
  private static calculateCommunicationScore(patterns: BehaviorPattern[], baseScore: number): number {
    const communicationPatterns = patterns.filter(p => 
      p.type === 'rapid_messaging' || p.type === 'spam'
    );
  ...
  private static calculateRespectScore(patterns: BehaviorPattern[], baseScore: number): number {
    const respectPatterns = patterns.filter(p => 
      p.type === 'harassment' || p.type === 'inappropriate_content'
    );
  ...
  private static calculateComplianceScore(patterns: BehaviorPattern[], baseScore: number): number {
    const compliancePatterns = patterns.filter(p => 
      p.type === 'inappropriate_content' || p.type === 'suspicious_requests'
    );
    return Math.max(0, baseScore - (compliancePatterns.length * 18));
  }
  
  private static calculateTrustworthinessScore(patterns: BehaviorPattern[], baseScore: number): number {
    const trustPatterns = patterns.filter(p => 
      p.type === 'spam' || p.type === 'suspicious_requests'
    );
