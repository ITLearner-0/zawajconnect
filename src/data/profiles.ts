import { DatabaseProfile } from '@/types/profile';

// Dummy profiles for demo purposes
export const dummyProfiles: DatabaseProfile[] = [
  {
    id: 'user-1',
    full_name: 'Ahmed Khan',
    birth_date: '1990-05-15',
    gender: 'Male',
    location: 'New York, USA',
    education_level: 'Masters',
    occupation: 'Software Engineer',
    religious_practice_level: 'Practicing',
    prayer_frequency: 'Five times daily',
    bio: 'I am a practicing Muslim looking for a compatible spouse who shares my values.',
    email_verified: true,
    phone_verified: true,
    id_verified: true,
    wali_name: undefined,
    wali_relationship: undefined,
    wali_contact: undefined,
    wali_verified: false,
    is_visible: true,
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1581092795360-fd1ca04f0952',
    gallery: [],
    privacy_settings: {
      profileVisibilityLevel: 1,
      showAge: true,
      showLocation: true,
      showOccupation: true,
      allowNonMatchMessages: true,
    },
    blocked_users: [],
    content_flags: [],
    moderation_status: 'approved',
    last_moderation_date: '2023-08-10',
    created_at: '2023-08-01T10:00:00Z',
    updated_at: '2023-08-10T15:30:00Z',
  },
  {
    id: 'user-2',
    full_name: 'Fatima Rahman',
    birth_date: '1992-09-20',
    gender: 'Female',
    location: 'Chicago, USA',
    education_level: 'Bachelors',
    occupation: 'Teacher',
    religious_practice_level: 'Very Practicing',
    prayer_frequency: 'Five times daily',
    bio: 'I am a Muslim teacher who loves to help children learn and grow.',
    email_verified: true,
    phone_verified: true,
    id_verified: true,
    wali_name: 'Ibrahim Rahman',
    wali_relationship: 'Father',
    wali_contact: '+1234567890',
    wali_verified: true,
    is_visible: true,
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1649972904349-6e44c42644a7',
    gallery: [],
    privacy_settings: {
      profileVisibilityLevel: 2,
      showAge: false,
      showLocation: true,
      showOccupation: true,
      allowNonMatchMessages: false,
    },
    blocked_users: [],
    content_flags: [],
    moderation_status: 'approved',
    last_moderation_date: '2023-08-12',
    created_at: '2023-08-02T09:15:00Z',
    updated_at: '2023-08-12T14:20:00Z',
  },
  {
    id: 'user-3',
    full_name: 'Yusuf Ali',
    birth_date: '1989-03-10',
    gender: 'Male',
    location: 'London, UK',
    education_level: 'PhD',
    occupation: 'Doctor',
    religious_practice_level: 'Practicing',
    prayer_frequency: 'Five times daily',
    bio: 'I am a doctor who believes in helping others and living by Islamic principles.',
    email_verified: true,
    phone_verified: true,
    id_verified: true,
    wali_name: undefined,
    wali_relationship: undefined,
    wali_contact: undefined,
    wali_verified: false,
    is_visible: true,
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1581092795360-fd1ca04f0952',
    gallery: [],
    privacy_settings: {
      profileVisibilityLevel: 1,
      showAge: true,
      showLocation: true,
      showOccupation: true,
      allowNonMatchMessages: true,
    },
    blocked_users: [],
    content_flags: [],
    moderation_status: 'approved',
    last_moderation_date: '2023-07-22',
    created_at: '2023-07-01T11:30:00Z',
    updated_at: '2023-07-22T16:45:00Z',
  },
  {
    id: 'user-4',
    full_name: 'Aisha Mahmood',
    birth_date: '1993-11-05',
    gender: 'Female',
    location: 'Toronto, Canada',
    education_level: 'Masters',
    occupation: 'Accountant',
    religious_practice_level: 'Moderately Practicing',
    prayer_frequency: 'Five times daily',
    bio: 'I enjoy reading, hiking, and spending time with family.',
    email_verified: true,
    phone_verified: true,
    id_verified: true,
    wali_name: 'Tariq Mahmood',
    wali_relationship: 'Brother',
    wali_contact: '+1987654321',
    wali_verified: true,
    is_visible: true,
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1649972904349-6e44c42644a7',
    gallery: [],
    privacy_settings: {
      profileVisibilityLevel: 1,
      showAge: true,
      showLocation: true,
      showOccupation: true,
      allowNonMatchMessages: true,
    },
    blocked_users: [],
    content_flags: [],
    moderation_status: 'approved',
    last_moderation_date: '2023-08-01',
    created_at: '2023-07-15T08:20:00Z',
    updated_at: '2023-08-01T13:10:00Z',
  },
  {
    id: 'user-5',
    full_name: 'Omar Farooq',
    birth_date: '1988-07-25',
    gender: 'Male',
    location: 'Sydney, Australia',
    education_level: 'Bachelors',
    occupation: 'Business Owner',
    religious_practice_level: 'Very Practicing',
    prayer_frequency: 'Five times daily',
    bio:
      'I run my own halal food business and am looking for someone who shares my entrepreneurial spirit.',
    email_verified: true,
    phone_verified: true,
    id_verified: true,
    wali_name: undefined,
    wali_relationship: undefined,
    wali_contact: undefined,
    wali_verified: false,
    is_visible: true,
    is_verified: true,
    profile_picture: 'https://images.unsplash.com/photo-1581092795360-fd1ca04f0952',
    gallery: [],
    privacy_settings: {
      profileVisibilityLevel: 1,
      showAge: true,
      showLocation: true,
      showOccupation: true,
      allowNonMatchMessages: true,
    },
    blocked_users: [],
    content_flags: [],
    moderation_status: 'approved',
    last_moderation_date: '2023-07-15',
    created_at: '2023-06-20T12:45:00Z',
    updated_at: '2023-07-15T17:30:00Z',
  },
];
