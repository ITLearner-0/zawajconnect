...
const ModerationTestSuite: React.FC = () => {
  ...
  const testCases: TestCase[] = [
    ...
    },
    {
      ...
      message:
        'Je suis ingénieur et je cherche une épouse pratiquante pour fonder une famille pieuse.',
      expectedAction: 'approved',
      expectedRules: [],
      category: 'Approprié',
      severity: 'low',
  ...
  const runAllTests = async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      try {
        ...
        const result = await moderateContent(testCase.message, 'automated-test', 'test-match-id');
  ...
  const runSingleTest = async (testCase: TestCase) => {
    ...
    try {
      const startTime = Date.now();
      const result = await moderateContent(testCase.message, 'single-test', 'test-match-id');
      const executionTime = Date.now() - startTime;
            ...
            return (
              <TabsTrigger key={category} value={category}>
                ...
                {testResults.length > 0 && (
                  <Badge
                    variant={
                      passed === total ? 'default' : passed > 0 ? 'secondary' : 'destructive'
                    }
                    className="ml-2"
                  >
                    {passed}/{total}
          ...
          <div className="space-y-4">
            {testCases.map((testCase) => {
              ...
              return (
                <Card
                  key={testCase.id}
                  className={`${result?.passed === false ? 'border-red-200 bg-red-50' : result?.passed === true ? 'border-green-200 bg-green-50' : ''}`}
                >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex gap-4 text-sm">
                          <span>
                            Attendu: <Badge variant="outline">{testCase.expectedAction}</Badge>
                          </span>
                          {result && (
                            <span>
                              Obtenu:{' '}
