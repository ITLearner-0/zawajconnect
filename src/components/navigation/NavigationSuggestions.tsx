...
const NavigationSuggestions = () => {
  ...
  const generateSuggestions = () => {
    ...
    if (currentRoute?.category === 'main') {
      contextualRoutes = allRoutes.filter(route => 
        route.category === 'matching' || route.category === 'tools'
      );
    } else if (currentRoute?.category === 'matching') {
      contextualRoutes = allRoutes.filter(route => 
        route.category === 'main' && route.path !== '/dashboard'
      );
    }
    ...
    const newSuggestions = [
      ...
      arr.findIndex(r => r?.path === route?.path) === index
  ...
  const getSuggestionLabel = (type: string) => {
    switch (type) {
      ...
      case 'frequent': return 'Populaire';
      case 'contextual': return 'Suggéré';
      default: return '';
    }
  };
  ...
  const getSuggestionColor = (type: string) => {
    switch (type) {
      case 'recent': return 'bg-blue-500/10 text-blue-700 dark:text-blue-300';
      case 'frequent': return 'bg-green-500/10 text-green-700 dark:text-green-300';
      case 'contextual': return 'bg-purple-500/10 text-purple-700 dark:text-purple-300';
