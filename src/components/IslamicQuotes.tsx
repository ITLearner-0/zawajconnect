...
import { BookOpen, RefreshCw, Heart, Share2, Copy, Star, Quote } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" onClick={getRandomQuote} disabled={loading}>
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Heart
                    className={`h-4 w-4 ${favorites.includes(currentQuote.id) ? 'fill-current' : ''}`}
                  />
                </Button>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                <div className="space-y-1">
                  <p className="font-medium text-primary">
                    {currentQuote.source}
                    {currentQuote.author && (
                      <span className="text-muted-foreground ml-2">- {currentQuote.author}</span>
                    )}
                  </p>

                  {currentQuote.reference && (
                    <p className="text-sm text-muted-foreground">{currentQuote.reference}</p>
                  )}
                </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          <Heart
            className={`h-4 w-4 mr-2 ${favorites.includes(currentQuote.id) ? 'fill-current text-red-500' : ''}`}
          />
