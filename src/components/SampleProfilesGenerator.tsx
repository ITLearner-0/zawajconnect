...
const sampleProfiles = [
  {
    ...
    looking_for:
      'Un homme pratiquant, bienveillant et ambitieux pour construire ensemble une famille pieuse.',
  ...
  },
  {
    ...
    looking_for:
      'Une épouse pieuse et éduquée pour fonder une famille selon les préceptes islamiques.',
  ...
  const generateSampleProfiles = async () => {
    ...
    try {
      for (const profile of sampleProfiles) {
        ...
        const { error: prefsError } = await supabase.from('islamic_preferences').insert({
          user_id: userId,
    ...
    <Card className="border-dashed border-2 border-emerald/30">
      ...
      <CardContent>
        <p className="text-muted-foreground mb-4">
          Ajoutez des profils d'exemple pour tester les fonctionnalités de matching et de
          navigation.
        </p>
        <Button
