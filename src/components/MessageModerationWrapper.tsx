...
import {
  useIslamicModeration,
  ModerationSuggestion as ModerationSuggestionType,
} from '@/hooks/useIslamicModeration';
...
const MessageModerationWrapper: React.FC<MessageModerationWrapperProps> = ({
  ...
  const handleAcceptSuggestion = async (suggestionId: string, newMessage: string) => {
    ...
    window.dispatchEvent(
      new CustomEvent('useSuggestedMessage', {
  ...
  return (
    <div className="space-y-4">
      ...
      {lastResult && !lastResult.approved && (
        <Card
          className={`border-${lastResult.action === 'blocked' ? 'red' : 'amber'}-200 bg-${lastResult.action === 'blocked' ? 'red' : 'amber'}-50/50`}
        >
          <CardContent className="p-3">
            <div className="flex items-start gap-2">
              <AlertCircle
                className={`h-4 w-4 text-${lastResult.action === 'blocked' ? 'red' : 'amber'}-500 mt-0.5`}
              />
              <div>
                <p
                  className={`text-sm font-medium text-${lastResult.action === 'blocked' ? 'red' : 'amber'}-700`}
                >
                  {lastResult.action === 'blocked'
                    ? 'Message bloqué'
                    : lastResult.action === 'warned'
                      ? 'Message nécessite attention'
                      : 'En révision'}
                </p>
                <p
                  className={`text-xs text-${lastResult.action === 'blocked' ? 'red' : 'amber'}-600 mt-1`}
                >
                  {lastResult.reason}
                </p>
