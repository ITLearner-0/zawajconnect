...
const ProfileCompletionGuide: React.FC<ProfileCompletionGuideProps> = ({
  ...
  const completionItems = [
    ...
    },
    {
      ...
      completed: formData.gender === 'male' || !!(formData.waliName && formData.waliRelationship && formData.waliContact),
          ...
          {completionItems.map((item) => (
            <div key={item.key} className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-medium ${item.completed ? 'text-green-700' : 'text-gray-900'}`}>
                  {item.label}
                </div>
