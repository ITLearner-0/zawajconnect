          ...
          <div className="flex items-center gap-2">
            ...
            {question.description && (
              <Tooltip>
                ...
                <TooltipContent className="max-w-xs">{question.description}</TooltipContent>
              </Tooltip>
            )}
          </div>
          ...
          <TabsContent value="response" className="py-6 space-y-6">
              ...
              <div className="grid grid-cols-5 gap-2 mb-4">
                {agreementLevels.map((level) => (
                  <Button
                    key={level.value}
                    variant={
                      Math.abs(level.value - (answer?.value || 60)) <= 10 ? 'default' : 'outline'
                    }
                    size="sm"
            ...
            <div className="bg-rose-50 dark:bg-rose-900/20 p-4 rounded-lg border border-rose-200 dark:border-rose-700">
              <p className="text-rose-800 dark:text-rose-200 text-sm">
                ...
                Cette réponse sera utilisée pour calculer votre compatibilité avec de futurs
                partenaires.
              </p>
            </div>
          </TabsContent>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs text-gray-500">
                    Par défaut: {((question.weight / 2) * 10).toFixed(1)}
                  </span>
                </div>
                <Slider
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  Augmentez l'importance pour que cette question ait plus d'influence sur votre
                ...
                {isDealbreaker && (
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-amber-700 dark:text-amber-300">
                      Les partenaires avec des réponses en dessous de ce seuil seront exclus de vos
                      résultats
                    </p>
                  </div>
                )}
