...
const emailTypeConfig: Record<string, { label: string; variant: 'default' | 'destructive' | 'outline' | 'secondary' }> = {
          ...
          {emails.map((email) => {
            ...
            return (
              <TableRow key={email.id}>
                <TableCell>
                  <Badge variant={emailType.variant}>
                    {emailType.label}
                  </Badge>
                </TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <StatusIcon className={`h-4 w-4 ${statusInfo?.className ?? 'text-muted-foreground'}`} />
