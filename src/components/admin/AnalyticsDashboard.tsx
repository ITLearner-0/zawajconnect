      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            ...
            <Progress
              value={
                ((userActivityStats?.activeUsers ?? 0) / (userActivityStats?.totalUsers ?? 1)) * 100
              }
              className="h-1 mt-3"
            />
            <p className="text-xs text-muted-foreground mt-2">
              {(
                ((userActivityStats?.activeUsers ?? 0) / (userActivityStats?.totalUsers ?? 1)) *
                100
              ).toFixed(1)}
              % of total users
            </p>
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <Progress
              value={
                ((waliStats?.supervisedConversations ?? 0) / (analytics?.totalConversations ?? 1)) *
                100
              }
              className="h-1 mt-3"
            />
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="flex items-center gap-2 mt-3">
              <Badge
                variant={(emergencyStats?.pendingReports ?? 0) > 0 ? 'destructive' : 'outline'}
                className={
                  (emergencyStats?.pendingReports ?? 0) > 0
                    ? ''
