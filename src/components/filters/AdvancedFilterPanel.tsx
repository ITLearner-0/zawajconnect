      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {filters.compatibilityScore?.enabled && (
            <div className="space-y-2">
              <Label>
                Score Range: {filters.compatibilityScore?.minScore || 0}% -{' '}
                {filters.compatibilityScore?.maxScore || 100}%
              </Label>
              <Slider
                value={[
                  filters.compatibilityScore?.minScore || 0,
                  filters.compatibilityScore?.maxScore || 100,
                ]}
                min={0}
                max={100}
            ...
            <>
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    updateDealBreakerFilter({ strictMode: value === 'strict' })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
                ...
                <div className="flex flex-wrap gap-2">
                  {DEALBREAKER_CATEGORIES.map((category) => (
                    <Badge
                      key={category}
                      variant={
                        filters.dealBreakers?.categories?.includes(category) ? 'default' : 'outline'
                      }
