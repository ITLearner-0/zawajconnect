...
const AdvancedIslamicFiltering = () => {
  ...
  const applyIslamicFilters = async () => {
    ...
    try {
      ...
      const { data: islamicPrefsData } = await supabase.from('islamic_preferences').select('*');
      ...
      const profiles: ProfileWithPreferences[] = profilesData
        .map((profile) => {
      ...
      const filtered = profiles.filter((profile) => {
        ...
        if (
          profile.age &&
          (profile.age < filters.age_range[0] || profile.age > filters.age_range[1])
        ) {
          return false;
        }
        ...
        if (
          filters.prayer_frequency.length > 0 &&
        ...
        if (
          filters.quran_reading.length > 0 &&
        ...
        // Sect filter
        if (filters.sect.length > 0 && !filters.sect.includes(islamicPrefs.sect || '')) {
          return false;
        }

        // Madhab filter
        if (filters.madhab.length > 0 && !filters.madhab.includes(islamicPrefs.madhab || '')) {
          return false;
        }

        // Hijab preference filter
        if (
          filters.hijab_preference.length > 0 &&
        ...
        if (
          filters.beard_preference.length > 0 &&
        ...
        // Halal diet filter
        if (filters.halal_diet !== null && islamicPrefs.halal_diet !== filters.halal_diet) {
          return false;
        }

        // Smoking filter
        if (filters.smoking.length > 0 && !filters.smoking.includes(islamicPrefs.smoking || '')) {
          return false;
        }

        // Importance of religion filter
        if (
          filters.importance_of_religion.length > 0 &&
  ...
  const getMatchingCriteria = (islamicPrefs: IslamicPreferences | undefined): string[] => {
    ...
    if (islamicPrefs?.importance_of_religion === 'very_important')
      criteria.push('Religion très importante');
                    ...
                    <div key={freq} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateArrayFilter('prayer_frequency', freq, checked as boolean)
                        }
                      />
                  ...
                  {['daily', 'weekly', 'monthly', 'rarely', 'never'].map((freq) => (
                    <div key={freq} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateArrayFilter('quran_reading', freq, checked as boolean)
                        }
                      />
                  ...
                  {['sunni', 'shia', 'other'].map((sect) => (
                    <div key={sect} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateArrayFilter('sect', sect, checked as boolean)
                        }
                      />
                      <label htmlFor={`sect-${sect}`} className="text-sm">
                        {sect === 'sunni' ? 'Sunnite' : sect === 'shia' ? 'Chiite' : 'Autre'}
                      </label>
                    </div>
                  ))}
                </div>
                  ...
                  {['hanafi', 'maliki', 'shafi', 'hanbali', 'salafi', 'other'].map((madhab) => (
                    <div key={madhab} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateArrayFilter('madhab', madhab, checked as boolean)
                        }
                      />
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {['never', 'rarely', 'socially', 'regularly'].map((smoking) => (
                    <div key={smoking} className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateArrayFilter('smoking', smoking, checked as boolean)
                        }
                      />
              ...
              <div>
                ...
                <div className="space-y-2">
                  {['very_important', 'important', 'moderate', 'not_important'].map(
                    (importance) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <Slider
                  ...
                  onValueChange={(value) =>
                    setFilters((prev) => ({ ...prev, age_range: value as [number, number] }))
                  }
                  max={60}
                  min={18}
                  step={1}
              ...
              <div>
                ...
                <Slider
                  ...
                  onValueChange={(value) =>
                    setFilters((prev) => ({ ...prev, location_distance: value[0] }))
                  }
                  max={200}
                  min={5}
                  step={5}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div
                        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium border ${getScoreColor(profile.islamic_score)}`}
                      >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm">Montrer l'intérêt</Button>
                  </div>
                </div>
              ))}
            </div>
