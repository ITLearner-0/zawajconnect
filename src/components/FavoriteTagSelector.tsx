...
interface FavoriteTagSelectorProps {
  profileId: string;
}

const FavoriteTagSelector = ({ profileId }: FavoriteTagSelectorProps) => {
  const { tags, addTagToFavorite, removeTagFromFavorite, getFavoriteTags } = useProfileTags();
  ...
  const handleTagToggle = async (tag: ProfileTag) => {
    ...
    if (isSelected) {
      const success = await removeTagFromFavorite(profileId, tag.id);
      if (success) {
        setSelectedTags(prev => prev.filter(t => t.id !== tag.id));
      }
    } else {
      const success = await addTagToFavorite(profileId, tag.id);
      if (success) {
...
export default FavoriteTagSelector;
