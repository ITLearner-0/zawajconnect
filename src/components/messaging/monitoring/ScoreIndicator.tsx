...
const ScoreIndicator: React.FC<ScoreIndicatorProps> = ({
  ...
  const getScoreIcon = (score: number) => {
    if (score >= 80)
      return (
        <ThumbsUp
          className={`${size === 'large' ? 'h-6 w-6' : 'h-4 w-4'} text-green-600 dark:text-green-400`}
        />
      );
    if (score >= 60)
      return (
        <Signal
          className={`${size === 'large' ? 'h-6 w-6' : 'h-4 w-4'} text-islamic-brightGold dark:text-islamic-darkBrightGold`}
        />
      );
    return (
      <AlertTriangle
        className={`${size === 'large' ? 'h-6 w-6' : 'h-4 w-4'} text-red-600 dark:text-red-400`}
      />
    );
  };
  ...
  if (size === 'large') {
    return (
      <div className="flex flex-col items-center">
        <div
          className={`${sizeClasses[size]} ${getScoreColor(score)} flex items-center gap-2 mb-2`}
        >
          {score}%{showIcon && getScoreIcon(score)}
        </div>
        {label && (
          <div className="text-sm text-islamic-burgundy dark:text-islamic-cream/70 mb-1">
            {label}
          </div>
        )}
        <Progress
