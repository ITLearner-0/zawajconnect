...
const Messages = () => {
  ...
  const [actualConversationId, setActualConversationId] = useState<string | undefined>(
    conversationId
  );
  ...
  const { isDemoConversation, demoMessages, setDemoMessages } =
    useDemoMessages(actualConversationId);
  ...
  useEffect(() => {
    ...
    if (currentError && currentError !== prevErrorRef.current && !toastShownRef.current) {
