...
export const useSecurityValidation = () => {
  ...
  const validateFamilyOperation = useCallback(
    async (
  ...
  const validateMessagePermissions = useCallback(
    async (matchId: string, requiredScore: number = 60): Promise<ValidationResult> => {
      if (!user || !securityStatus) {
        return {
          isValid: false,
  ...
  const validateProfileAccess = useCallback(
    async (targetUserId: string, requiredScore: number = 85): Promise<ValidationResult> => {
      if (!user || !securityStatus) {
        return {
          isValid: false,
          reason: 'Utilisateur non authentifié',
  ...
  const validateWithFeedback = useCallback(
    async (validationFn: () => Promise<ValidationResult>): Promise<boolean> => {
      setValidating(true);
      try {
