...
export const useOnboardingSuggestions = () => {
  ...
  const generateSuggestions = useCallback(
    async (type: SuggestionType, data: Record<string, any>): Promise<SuggestionResult | null> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (result?.error) {
          ...
          if (result.error.includes('Limite de requêtes')) {
            toast({
              ...
              description:
                'Veuillez attendre un moment avant de demander de nouvelles suggestions.',
              variant: 'destructive',
            });
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Erreur lors de la génération des suggestions';
  ...
  const generateBioSuggestions = useCallback(
    (data: {
      interests?: string[];
      profession?: string;
      education?: string;
      lookingFor?: string;
    }) => generateSuggestions('bio', data),
    [generateSuggestions]
  );

  const generateIslamicPreferencesSuggestions = useCallback(
    (data: {
      sect?: string;
      madhab?: string;
    ...
    }) => generateSuggestions('islamic_preferences', data),
    [generateSuggestions]
  );

  const generateFieldTips = useCallback(
    (data: { fieldName: string; currentValue?: string; context?: string }) =>
      generateSuggestions('field_tips', data),
    [generateSuggestions]
  );

  const generateInterestsSuggestions = useCallback(
    (data: { currentInterests?: string[]; profession?: string; education?: string }) =>
      generateSuggestions('interests_suggestions', data),
    [generateSuggestions]
  );
