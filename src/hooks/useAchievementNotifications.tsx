...
const AchievementNotification = ({ achievement }: AchievementNotificationProps) => {
  const getRarityColor = (rarity: string) => {
    switch (rarity) {
      ...
      case 'epic': return 'from-purple-400 via-pink-300 to-purple-500';
      case 'rare': return 'from-blue-400 via-cyan-300 to-blue-500';
      default: return 'from-emerald-400 via-green-300 to-emerald-500';
    }
  };
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl p-4 bg-gradient-to-r ${getRarityColor(achievement.rarity)} shadow-2xl border-2 border-white/20`}>
...
const playAchievementSound = (rarity: string) => {
  ...
  const frequencies = {
    ...
    legendary: [1046.50, 1318.51, 1567.98] // C6, E6, G6
  };
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          toast.custom(
            (t) => <AchievementNotification achievement={achievement} />,
            {
              duration: achievement.rarity === 'legendary' ? 6000 : 
                       achievement.rarity === 'epic' ? 5000 : 
                       achievement.rarity === 'rare' ? 4000 : 3000,
              position: 'top-center',
            }
          );
        }
  ...
  return {
    ...
    triggerTestNotification: (rarity: 'common' | 'rare' | 'epic' | 'legendary' = 'common') => {
      const testAchievement: AchievementUnlock = {
        ...
        points_awarded: rarity === 'legendary' ? 500 : rarity === 'epic' ? 200 : rarity === 'rare' ? 100 : 50,
        rarity,
      ...
      toast.custom(
        (t) => <AchievementNotification achievement={testAchievement} />,
        {
          duration: rarity === 'legendary' ? 6000 : 
                   rarity === 'epic' ? 5000 : 
                   rarity === 'rare' ? 4000 : 3000,
          position: 'top-center',
        }
      );
    }
