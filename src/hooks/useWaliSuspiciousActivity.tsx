...
export const useWaliSuspiciousActivity = () => {
   ...
   */
  const detectConsecutiveRejections = useCallback(async (
    userId: string,
    threshold: number = 5
   ...
   */
  const detectRepeatedProfileViews = useCallback(async (
    userId: string,
    threshold: number = 5,
      ...
      if (data && data.length > 0) {
        return data
          ...
          .map((item: any) => ({
            ...
            severity: item.view_count >= 10 ? 'critical' : item.view_count >= 7 ? 'high' : 'medium',
   ...
   */
  const detectRateLimitViolations = useCallback(async (
    userId: string
  ): Promise<SuspiciousPattern[]> => {
    try {
   ...
   */
  const analyzeActivity = useCallback(async (userId: string) => {
    try {
      setChecking(true);
   ...
   */
  const createAdminAlert = useCallback(async (
    userId: string,
   ...
   */
  const getWaliAlerts = useCallback(async (
    userId: string,
    limit: number = 20
  ) => {
    try {
