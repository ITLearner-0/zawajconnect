...
export function validateAnswerValue(value: unknown, questionId: string): AnswerValue {
  ...
  if (answerValue.value < 0 || answerValue.value > 100) {
    throw new ValidationError(
      'Answer value must be between 0 and 100',
      `${questionId}.value`,
      answerValue.value
    );
  }
  ...
  if (answerValue.isBreaker !== undefined) {
    if (typeof answerValue.isBreaker !== 'boolean') {
      throw new ValidationError(
        'isBreaker must be a boolean',
        `${questionId}.isBreaker`,
        answerValue.isBreaker
      );
    }
  ...
  if (answerValue.breakerThreshold !== undefined) {
    if (
      typeof answerValue.breakerThreshold !== 'number' ||
      answerValue.breakerThreshold < 0 ||
      answerValue.breakerThreshold > 100
    ) {
      throw new ValidationError(
        'breakerThreshold must be a number between 0 and 100',
        `${questionId}.breakerThreshold`,
        answerValue.breakerThreshold
      );
    }
...
export function validateUserPreferences(preferences: unknown): UserPreferences {
  if (!preferences || typeof preferences !== 'object') {
    logWarning('validateUserPreferences', 'Invalid preferences format, using defaults', {
      preferences,
    });
    return { categories: [] };
  }
  ...
  if (Array.isArray(prefsObj.categories)) {
    result.categories = prefsObj.categories.filter(
      (cat): cat is { category: string; weight: number } => {
        return (
          cat &&
          typeof cat === 'object' &&
  ...
  if (Array.isArray(prefsObj.dealbreakers)) {
    result.dealbreakers = prefsObj.dealbreakers.filter(
  ...
  if (
    typeof prefsObj.minCompatibilityScore === 'number' &&
