...
export class MatchQualityService {
  calculateMatchQuality(
    ...
    const improvementSuggestions = this.generateImprovementSuggestions(myResults, otherUser, compatibilityMatch);
  ...
  private generateCompatibilityReasons(match: CompatibilityMatch): string[] {
    ...
    if (match.profileData) {
      if (match.profileData.religious_practice_level) {
        reasons.push(`Shared religious practice level: ${match.profileData.religious_practice_level}`);
      }
  ...
  private getAnswerQuality(myResults: ValidatedUserResults, otherUser: UserResultWithProfile): number {
