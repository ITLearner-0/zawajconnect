...
export const useConversationStatus = () => {
  ...
  const checkIfInConversation = useCallback(
  ...
  const endConversation = useCallback(
    async (
      ...
      try {
        ...
        const otherUserId =
          (matchData as any).user1_id === user.id
            ? (matchData as any).user2_id
            : (matchData as any).user1_id;

        // 2. Message islamique automatique
        const islamicMessage =
          "🤲 Qu'Allah vous guide vers ce qui est meilleur pour vous et facilite votre chemin vers un mariage béni. Que cette recherche sincère soit acceptée et que vous trouviez la quiétude du cœur. Ameen.";
        ...
        const { error: blockError } = await supabase.from('blocked_match_pairs' as any).insert({
        ...
        toast({
          ...
          description:
            "L'échange a été clôturé avec respect. Vous pouvez maintenant chercher de nouveaux profils.",
        });
  ...
  const hasPreviousConversation = useCallback(
