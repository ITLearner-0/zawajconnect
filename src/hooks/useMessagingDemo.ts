...
export const useMessagingDemo = () => {
  ...
  const sendMessage = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setConversations((prev) =>
        prev.map((c) => (c.id === currentConversation.id ? { ...c, last_message: newMessage } : c))
      );
  ...
  const updateRetentionPolicy = useCallback((policy: RetentionPolicy) => {
    ...
    toast.info(
      `Message retention policy updated: ${policy.type === 'temporary' ? `${policy.duration_days} days` : 'permanent'}`
    );
  }, []);
