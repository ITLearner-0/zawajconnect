...
export const useFamilyApproval = () => {
  ...
  const loadNotifications = async (userId?: string): Promise<void> => {
    ...
    try {
      ...
      const normalizedNotifications: EnrichedFamilyNotification[] = (data ?? []).map(
        (item): EnrichedFamilyNotification => {
  ...
  const handleApprovalDecision = async (
    ...
    try {
      ...
      const { error: reviewError } = await supabase.from('family_reviews').insert(reviewData);
      ...
      toast({
        title: 'Erreur',
        description:
          error instanceof Error ? error.message : "Impossible d'enregistrer votre décision",
        variant: 'destructive',
      });
    } finally {
